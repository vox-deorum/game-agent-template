...
Spades is a four-player partnership card game. You and the player across the table form a team. Your team bids how many tricks it expects to take, then tries to take that many and score more points than the other team. The [agent interface](https://vox-deorum.github.io/game-sandbox/students/agent-interface/) explains the `reset` and `act` methods shared by every environment. This page covers everything specific to Spades.

After you complete [Getting Started](https://vox-deorum.github.io/game-sandbox/students/getting-started/), open `agent.py`, run `python -m sandbox watch`, then follow [Your first agent](#your-first-agent).
...
Messages use a platform player ID string such as `"player_2"`, not the player index in the observation. The example builds the platform player ID from the partner index and saves it during `act`, because `chat` does not receive the observation. A message can hold up to 120 characters by default (some emoji count as more than one). A season can lower that cap or disable messaging, but cannot raise it. See the [agent interface](https://vox-deorum.github.io/game-sandbox/students/agent-interface/#chatinbox) for delivery timing, broadcast and targeted messages, replay visibility, and how chat time counts toward your limits.
