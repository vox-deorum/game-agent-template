...
Hearts is a four-player card game where your goal is to take as few penalty points as possible. Your agent controls one player. The [agent interface](https://vox-deorum.github.io/game-sandbox/students/agent-interface/) explains the `reset` and `act` methods shared by every environment. This page covers everything specific to Hearts.
