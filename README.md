...
The [getting started guide](https://vox-deorum.github.io/game-sandbox/students/getting-started/) explains manual virtual-environment setup and the GitHub workflow.

## Write the agent

Open `agent.py` and implement the required `reset(seed, observation)` and `act(observation)` methods. `act` runs on your turn and returns the card you want to play. Read [`environment.md`](environment.md) before you start; it explains the starter agent, rules, observations, `sandbox.cards` helpers, scoring, and time limits. The optional `learn` and `chat` methods are described in the [agent interface reference](https://vox-deorum.github.io/game-sandbox/students/agent-interface/) with full signatures; Hearts ships with messaging off. Leave an optional method out when you do not use it.

The template already plays its lowest-ranked legal card. `TODO(you)` in `act` marks where to improve it.

## Submit

Submit the repository URL through the course website. The [submitting guide](https://vox-deorum.github.io/game-sandbox/students/submitting/) covers validation and common errors.
